...
$Version = '1.36'
$Revision = '1.36-stress-runner-runtime-safety'
...
try {
    ...
    Info '[5/7] Building FolderHeatMap 1.36 and tools...'; Run-Native -Phase 'BUILD' -Exe $cmake -ArgumentList @('--build','build','--config','Release','--target','FolderHeatMap','FolderHeatMapEngine','FolderHeatMapConfig','FolderHeatMapReset')|Out-Null
    ...
    foreach ($f in @('configure.cmd','README.md','TESTING.md','STRESS_TESTING.md','test.cmd','test.ps1','test_stress.ps1')) { if (Test-Path (Join-Path $Repo $f)) { Copy-Item -LiteralPath (Join-Path $Repo $f) -Destination (Join-Path $package $f) -Force } }
    ...
    foreach ($f in @('configure.cmd','README.md','TESTING.md','STRESS_TESTING.md','test.cmd','test.ps1','test_stress.ps1')) { if (Test-Path (Join-Path $package $f)) { Copy-FileWithRetry -Source (Join-Path $package $f) -Destination (Join-Path $dist $f) -Phase 'DEPLOY' } }
