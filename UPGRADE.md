...
- **Managed host configuration duplicates on repeated installs:** a repair/install step that only appends or only inspects the first matching entry can create a new duplicate on every run. Symptom: menus accumulate repeated project-owned views/rules even though the installer reports success. Prevention: treat managed host configuration as a set with a single canonical entry; scan all matching entries, preserve exactly one, remove stale duplicates, preserve unrelated user entries, and verify the final count after every install/upgrade.
...
Before declaring an upgrader stable, test: clean repo/app stopped; clean repo/app running; old local updater; no update; remote update; build failure; locked deployment target; graceful shutdown timeout; existing runtime config/log/database; tracked local modification; untracked runtime file; repository path with spaces; repository on a mapped/UNC network path that triggers Git `dubious ownership`; optional third-party host update preserving the exact existing installation directory and required host configuration; repeated install/upgrade with exactly one instance of every project-managed host configuration entry; immediate second run; final log status and exit code for success/warning/failure.
