...
function Resolve-TC {
    ...
    if (-not $tcPath) { foreach($k in $keys){$v=Get-RegValue $k 'InstallDir';if($v){$tcPath=[Environment]::ExpandEnvironmentVariables([string]$v);break}} }
    if (-not $tcIni) { foreach($k in $keys){$v=Get-RegValue $k 'IniFileName';if($v){$tcIni=[Environment]::ExpandEnvironmentVariables([string]$v);break}} }
    $exe=$null
...
public static class FhmStressNative {
  ...
  public static string FileIdentity(string path,bool directory) {
    ...
    using(SafeFileHandle h=CreateFileW(path,ACCESS,SHARE,IntPtr.Zero,OPEN,directory?BACKUP:NORMAL,IntPtr.Zero)) {
      if(h.IsInvalid) return null; FILE_ID_INFO i;
      if(!GetFileInformationByHandleEx(h,18,out i,(uint)Marshal.SizeOf(typeof(FILE_ID_INFO))) || i.FileId.Identifier==null) return null;
      return i.VolumeSerialNumber.ToString()+":"+BitConverter.ToString(i.FileId.Identifier).Replace("-","").ToLowerInvariant();
    }
  }
}
'@
...
function Relative-Path([string]$Path) { $full=[IO.Path]::GetFullPath($Path).Replace('/','\').TrimEnd('\'); if($full.Length-lt3-or$full[1]-ne':'){throw "Only drive-letter paths are supported: $full"}; return $full.Substring(3).ToLowerInvariant() }
function Invoke-SqlRow([string]$Database,[string]$Sql,[string[]]$Columns) {
    [IntPtr]$db=[IntPtr]::Zero; [IntPtr]$st=[IntPtr]::Zero
    $rc=[FhmStressNative]::sqlite3_open16($Database,[ref]$db); if($rc-ne0){throw "sqlite3_open16 failed rc=$rc"}
    try { $rc=[FhmStressNative]::sqlite3_prepare16_v2($db,$Sql,-1,[ref]$st,[IntPtr]::Zero); if($rc-ne0){throw "sqlite prepare failed rc=$rc"}; $rc=[FhmStressNative]::sqlite3_step($st); if($rc-ne100){return $null}; $o=[ordered]@{}; for($i=0;$i-lt$Columns.Count;$i++){ if([FhmStressNative]::sqlite3_column_type($st,$i)-eq1){$o[$Columns[$i]]=[FhmStressNative]::sqlite3_column_int64($st,$i)}else{$o[$Columns[$i]]=$null} }; return [pscustomobject]$o }
    finally { if($st-ne[IntPtr]::Zero){[void][FhmStressNative]::sqlite3_finalize($st)}; if($db-ne[IntPtr]::Zero){[void][FhmStressNative]::sqlite3_close($db)} }
}
function Get-FolderState([string]$Database,[string]$Path) { $rel=SqlEscape(Relative-Path $Path); Invoke-SqlRow $Database "SELECT f.visits,f.last_visit,u.heat_visits,u.recent_visits,u.active_days,u.first_active_day,u.last_active_day,u.last_effective_visit FROM folders f LEFT JOIN folder_usage u ON u.storage_key=f.storage_key WHERE lower(f.relative_path)='$rel' LIMIT 1;" @('visits','last_visit','heat_visits','recent_visits','active_days','first_active_day','last_active_day','last_effective_visit') }
function Get-FileState([string]$Database,[string]$Path) { $rel=SqlEscape(Relative-Path $Path); Invoke-SqlRow $Database "SELECT write_events,last_write,active_days,first_active_day,last_active_day FROM file_activity WHERE lower(relative_path)='$rel' LIMIT 1;" @('write_events','last_write','active_days','first_active_day','last_active_day') }
function State-Signature($State,[string[]]$Fields) { if($null-eq$State){return'<null>'}; return ($Fields|ForEach-Object{"$_=$($State.$_)"})-join';' }
function Wait-Until([scriptblock]$Condition,[int]$TimeoutMs=12000,[int]$IntervalMs=250) { $deadline=[DateTime]::UtcNow.AddMilliseconds($TimeoutMs); do{$v=&$Condition;if($v){return$v};Start-Sleep -Milliseconds $IntervalMs}while([DateTime]::UtcNow-lt$deadline);return$null }
function Navigate-TC([string]$TcExe,[string]$Path) { [void](Assert-InWorkspace $Path); Info("[TC] Navigate left panel -> $Path"); $p=Start-Process -FilePath $TcExe -ArgumentList @('/O',('/L="{0}"'-f$Path)) -PassThru; if($p){$p.WaitForExit(5000)|Out-Null}; Start-Sleep -Milliseconds 500 }
function Heat-Directory([string]$TcExe,[string]$Parent,[string]$Path,[int]$Visits=3) { for($i=0;$i-lt$Visits;$i++){Navigate-TC $TcExe $Path;Start-Sleep -Milliseconds 1100;Navigate-TC $TcExe $Parent;Start-Sleep -Milliseconds 1100} }
function Heat-File([string]$Path,[int]$Writes=3) { for($i=1;$i-le$Writes;$i++){[IO.File]::AppendAllText((Assert-InWorkspace $Path),("stress-write-$i "+[DateTime]::Now.ToString('O')+"`r`n"),$Utf8);Start-Sleep -Milliseconds 1300} }
...
try {
    ...
    for($i=1;$i-le6;$i++){ $target=if($rapidCurrent.StartsWith($src,[StringComparison]::OrdinalIgnoreCase)){Join-Path $dst'RAPID_DIR'}else{Join-Path $src'RAPID_DIR'}; Move-Item -LiteralPath(Assert-InWorkspace $rapidCurrent)-Destination(Assert-InWorkspace $target)-Force; Start-Sleep -Milliseconds 450; $rapidCurrent=$target }
    ...
    Write-LogLine ''; Write-LogLine '============================================================'; Write-LogLine 'FolderHeatMap lifecycle stress test summary'; Write-LogLine("PASS:    $PassCount") Green; Write-LogLine("ERROR:   $ErrorCount")$(if($ErrorCount-gt0){[ConsoleColor]::Red}else{[ConsoleColor]::Green}); Write-LogLine("WARNING: $WarningCount")$(if($WarningCount-gt0){[ConsoleColor]::Yellow}else{[ConsoleColor]::Gray}); if($ErrorCount-eq0){Write-LogLine'RESULT: PASS' Green}else{Write-LogLine'RESULT: ERROR' Red}; Write-LogLine("Log: $LogPath") Gray; Write-LogLine '============================================================'
}
if($ErrorCount-gt0){exit 1};exit 0
