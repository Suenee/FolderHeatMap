...
function Relative-Path([string]$Path) { $full=[IO.Path]::GetFullPath($Path).Replace('/','\').TrimEnd('\'); if($full.Length -lt 3 -or $full[1] -ne ':'){throw "Only drive-letter paths are supported: $full"}; return $full.Substring(3).ToLowerInvariant() }
function Invoke-SqlRow([string]$Database,[string]$Sql,[string[]]$Columns) {
    [IntPtr]$db=[IntPtr]::Zero; [IntPtr]$st=[IntPtr]::Zero
    $rc=[FhmStressNative]::sqlite3_open16($Database,[ref]$db); if($rc -ne 0){throw "sqlite3_open16 failed rc=$rc"}
    try { $rc=[FhmStressNative]::sqlite3_prepare16_v2($db,$Sql,-1,[ref]$st,[IntPtr]::Zero); if($rc -ne 0){throw "sqlite prepare failed rc=$rc"}; $rc=[FhmStressNative]::sqlite3_step($st); if($rc -ne 100){return $null}; $o=[ordered]@{}; for($i=0;$i -lt $Columns.Count;$i++){ if([FhmStressNative]::sqlite3_column_type($st,$i) -eq 1){$o[$Columns[$i]]=[FhmStressNative]::sqlite3_column_int64($st,$i)}else{$o[$Columns[$i]]=$null} }; return [pscustomobject]$o }
