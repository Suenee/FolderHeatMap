...
try {
    ...
    Dump-EngineTrace @('DIAG_RENAME_DIR','DIAG_RENAMED_DIR')
    ...
    Dump-State 'FILE RENAME BEFORE' $renameFile $fileBefore $fileIdBefore $fileFields
    Dump-State 'FILE RENAME NEW' $renameFileNew $fileAfter $fileIdAfter $fileFields
    Info ('[DIAG] FILE RENAME old state=' + (State-Signature $fileOldAfter $fileFields))
    Dump-EngineTrace @('diag_rename_file.txt','diag_renamed_file.txt')
    ...
    Dump-State 'MOVE BEFORE' $moveFile $moveBefore $moveIdBefore $fileFields
    Dump-State 'MOVE AFTER' $moveFileDst $moveOnly $moveIdAfter $fileFields
    ...
    Dump-EngineTrace @('diag_move_write.txt')
}
catch {
