...
- Updated `install.cmd` to 1.09 and added a controlled local-WDX loading test for Total Commander. After the normal FolderHeatMap integration repair, the launcher now copies the verified `dist\FolderHeatMap.wdx64` to `D:\Temp\FolderHeatMap\FolderHeatMap.wdx64` and redirects the matching `[ContentPlugins]` slot there while preserving `[ContentPlugins64] <slot>=1`.
- Added `deploy_local_wdx.ps1` 1.00 as the internal helper behind `install.cmd`. It stops Total Commander before replacing the local WDX, verifies source/target SHA-256 equality, creates a timestamped `WINCMD.INI` backup, verifies the final x64 registration pair, restores the previous Total Commander running state, and logs the result to `logs\local_wdx_test.log`.
- This local WDX deployment is intentionally diagnostic: repository data, database data and FolderHeatMap network/NAS identity remain supported on mapped/UNC storage, while the WDX loader itself is isolated onto a local NTFS path to determine whether Total Commander/Windows loader behavior is the cause of the reported `FolderHeatMap.wdx64 was not found` error on a NAS-hosted repository.
