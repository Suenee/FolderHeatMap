...
try {
    ...
    TestHeader 'Directory DELETE -> RECREATE starts cold'
    Navigate-TC $tc.Exe $src
    $oldDirId = [FhmNative]::FileIdentity($hotDir,$true)
    $oldDirState = Get-FolderState $database $hotDir
    $logStart = if (Test-Path -LiteralPath $EngineLog) { (Get-Item -LiteralPath $EngineLog).Length } else { 0 }
    Remove-Item -LiteralPath (Assert-InWorkspace $hotDir) -Recurse -Force
    Pass 'Heated directory deleted.'
    $directoryPurged = Wait-Until { if (-not (Get-FolderState $database $hotDir)) { return $true }; return $null } 12000 300
    if ($directoryPurged) { Pass 'Deleted directory history was purged.' } else { ErrorResult 'Deleted directory history remained in the database.' }
    New-Item -ItemType Directory -Path (Assert-InWorkspace $hotDir) -Force | Out-Null
    $newDirId = [FhmNative]::FileIdentity($hotDir,$true)
    if ($oldDirId -and $newDirId -and $oldDirId -ne $newDirId) { Pass 'Recreated directory has a different File ID.' } else { ErrorResult ("Recreated directory identity did not change. old='$oldDirId' new='$newDirId'") }
    Navigate-TC $tc.Exe $hotDir
    Start-Sleep -Milliseconds 1200
    $newDirState = Wait-Until { Get-FolderState $database $hotDir } 12000 300
    if ($oldDirState -and $newDirState -and $newDirState.visits -lt $oldDirState.visits) {
        Pass ("Recreated directory starts with new history: Visits=$($newDirState.visits), old=$($oldDirState.visits)")
    } else {
        ErrorResult ("Recreated directory inherited old history. new=[" + (State-Signature $newDirState $folderFields) + "] old=[" + (State-Signature $oldDirState $folderFields) + "]")
    }
    Log-EngineTrace (Get-EngineLogTail $logStart)

    TestHeader 'File DELETE -> RECREATE starts cold'
    Navigate-TC $tc.Exe $src
    $oldFileId = [FhmNative]::FileIdentity($file,$false)
    $oldFileState = Get-FileState $database $file
    $logStart = if (Test-Path -LiteralPath $EngineLog) { (Get-Item -LiteralPath $EngineLog).Length } else { 0 }
    Remove-Item -LiteralPath (Assert-InWorkspace $file) -Force
    Pass 'Heated file deleted.'
    $filePurged = Wait-Until { if (-not (Get-FileState $database $file)) { return $true }; return $null } 12000 300
    if ($filePurged) { Pass 'Deleted file history was purged.' } else { ErrorResult 'Deleted file history remained in the database.' }
    [IO.File]::WriteAllText((Assert-InWorkspace $file),"recreated $RunId`r`n",$Utf8)
    $newFileId = [FhmNative]::FileIdentity($file,$false)
    if ($oldFileId -and $newFileId -and $oldFileId -ne $newFileId) { Pass 'Recreated file has a different File ID.' } else { ErrorResult ("Recreated file identity did not change. old='$oldFileId' new='$newFileId'") }
    Start-Sleep -Milliseconds 1300
    [IO.File]::AppendAllText((Assert-InWorkspace $file),"new-write`r`n",$Utf8)
    $newFileState = Wait-Until { Get-FileState $database $file } 12000 300
    if ($oldFileState -and $newFileState -and $newFileState.write_events -lt $oldFileState.write_events) {
        Pass ("Recreated file starts with new history: Writes=$($newFileState.write_events), old=$($oldFileState.write_events)")
    } else {
        ErrorResult ("Recreated file inherited old history. new=[" + (State-Signature $newFileState $fileFields) + "] old=[" + (State-Signature $oldFileState $fileFields) + "]")
    }
    Log-EngineTrace (Get-EngineLogTail $logStart)
