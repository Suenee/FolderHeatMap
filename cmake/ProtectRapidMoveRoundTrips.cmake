...
function(fhm_roundtrip_replace_once OLD NEW LABEL)
    ...
    if(POS EQUAL -1)
        message(FATAL_ERROR "1.47 rapid MOVE round-trip anchor not found: ${LABEL}")
    endif()
    string(REPLACE "${OLD}" "${NEW}" PATCHED "${ENGINE}")
    set(ENGINE "${PATCHED}" PARENT_SCOPE)
endfunction()

# 1.46 proved that a per-task hint is insufficient: after the first MOVE has
# migrated tracked_objects and consumed its hint, a later round-trip removal of
# the same old path can no longer recover the File ID. Keep the last confirmed
# canonical identity for observed paths independently of individual tasks.
...
message(STATUS "Injected FolderHeatMap 1.47 rapid MOVE round-trip File ID memory: ${INPUT}")
