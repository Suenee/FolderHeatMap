...
std::wstring QueryRegString(HKEY root, const wchar_t* subkey, const wchar_t* value) {
    wchar_t buffer[2048]{}; DWORD type = 0; DWORD size = sizeof(buffer);
...
void RemoveOldHeatSearches() {
    for (int i = 1; i <= 112; ++i) {
        wchar_t base[64]{}; swprintf_s(base, L"FolderHeatMap Heat %03d", i);
...
struct ExistingFilter { std::wstring value; std::wstring color; };

std::vector<ExistingFilter> ReadNonHeatFilters() {
    std::vector<ExistingFilter> filters;
    for (int i = 1; i <= 256; ++i) {
        wchar_t key[64]{}; swprintf_s(key, L"ColorFilter%d", i);
        wchar_t value[2048]{};
        GetPrivateProfileStringW(L"Colors", key, L"", value, 2048, g_wincmdIni.c_str());
        if (!*value) continue;
        if (wcsncmp(value, L">FolderHeatMap Heat ", 19) == 0) continue;
        wchar_t ckey[64]{}; swprintf_s(ckey, L"ColorFilter%dColor", i);
        wchar_t color[64]{}; GetPrivateProfileStringW(L"Colors", ckey, L"-1", color, 64, g_wincmdIni.c_str());
...
bool ApplyTcColorRules() {
    ...
    for (const auto& f : existing) {
        wchar_t key[64]{}; swprintf_s(key, L"ColorFilter%d", index);
    ...
    for (int step = 1; step <= maxStep; ++step) {
        wchar_t name[64]{}; swprintf_s(name, L"FolderHeatMap Heat %03d", step);
        const double heat = static_cast<double>(step) / steps;
        int upper = std::clamp(static_cast<int>(ceil(heat)), 1, 7);
        int lower = std::max(1, upper - 1);
        double t = upper == lower ? 0.0 : std::clamp(heat - lower, 0.0, 1.0);
        COLORREF color = upper == 1 ? static_cast<COLORREF>(g_settings.colors[1]) : Interpolate(static_cast<COLORREF>(g_settings.colors[lower]), static_cast<COLORREF>(g_settings.colors[upper]), t);
        ...
        std::wstring plugin = L"folderheatmap.Heat Color Step = " + std::to_wstring(step);
        WritePrivateProfileStringW(L"Searches", (prefix + L"_plugin").c_str(), plugin.c_str(), g_wincmdIni.c_str());

        wchar_t key[64]{}; swprintf_s(key, L"ColorFilter%d", index);
        std::wstring filter = L">" + prefix;
        WritePrivateProfileStringW(L"Colors", key, filter.c_str(), g_wincmdIni.c_str());
        swprintf_s(key, L"ColorFilter%dColor", index);
        wchar_t colorText[64]{}; swprintf_s(colorText, L"%lu", static_cast<unsigned long>(color));
...
int ReadEditInt(HWND h, int fallback, int minV, int maxV) {
    wchar_t b[64]{}; GetWindowTextW(h, b, 64);
    wchar_t* e=nullptr; long v=wcstol(b,&e,10);
    return e==b ? fallback : std::clamp(static_cast<int>(v), minV, maxV);
}

void RestartTcIfNeeded(bool wasRunning) {
...
void Apply(HWND hwnd) {
    ...
    SetWindowTextW(g_status, L"Uloženo. Pravidla barev byla zapsána do wincmd.ini.");
    RestartTcIfNeeded(wasRunning);
}
    ...
    switch (msg) {
        case WM_CREATE: {
            ...
            auto add = [&](const wchar_t* cls,const wchar_t* txt,DWORD style,int x,int y,int w,int h,int id=0){
                HWND c=CreateWindowExW(0,cls,txt,WS_CHILD|WS_VISIBLE|style,x,y,w,h,hwnd,reinterpret_cast<HMENU>(static_cast<INT_PTR>(id)),nullptr,nullptr);
            ...
            for(int i=1;i<=7;++i){ g_colorButtons[i]=add(L"BUTTON",L"",BS_PUSHBUTTON,30,258+(i-1)*34,220,28,IDC_COLOR_BASE+i); }
        ...
        case WM_COMMAND: {
            ...
            if(id>=IDC_COLOR_BASE+1 && id<=IDC_COLOR_BASE+7){
                int level=id-IDC_COLOR_BASE;
                static COLORREF custom[16]{};
                CHOOSECOLORW cc{sizeof(cc)}; cc.hwndOwner=hwnd; cc.rgbResult=static_cast<COLORREF>(g_settings.colors[level]); cc.lpCustColors=custom; cc.Flags=CC_FULLOPEN|CC_RGBINIT;
                if(ChooseColorW(&cc)){ g_settings.colors[level]=cc.rgbResult; UpdateColorButton(level); }
                return 0;
            }
            if(id==IDC_APPLY){ Apply(hwnd); return 0; }
            return 0;
        }
...
int WINAPI wWinMain(HINSTANCE instance,HINSTANCE,LPWSTR,int show) {
    ...
    WNDCLASSW wc{}; wc.lpfnWndProc=WndProc; wc.hInstance=instance; wc.lpszClassName=L"FolderHeatMapConfigWindow"; wc.hCursor=LoadCursor(nullptr,IDC_ARROW); wc.hbrBackground=reinterpret_cast<HBRUSH>(COLOR_WINDOW+1);
    RegisterClassW(&wc);
    HWND hwnd=CreateWindowExW(WS_EX_APPWINDOW,wc.lpszClassName,L"FolderHeatMap – Nastavení",WS_OVERLAPPED|WS_CAPTION|WS_SYSMENU|WS_MINIMIZEBOX,CW_USEDEFAULT,CW_USEDEFAULT,660,590,nullptr,nullptr,instance,nullptr);
    if(!hwnd) return 2;
